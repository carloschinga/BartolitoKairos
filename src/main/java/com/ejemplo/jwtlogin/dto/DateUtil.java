...
public class DateUtil {
	...
	public static long getDiasHastaSabado(LocalDateTime fechaActual) {
		...
		if (diaActual == DayOfWeek.SATURDAY && horaActual.isAfter(LocalTime.MIDNIGHT) && horaActual.isBefore(LocalTime.of(6, 0))) {
			return 0;
		}
	...
	public static List<LocalDateTime> getFechasDelMesHastaHoy(LocalDateTime fechaFin) {
	 ...
	 public static List<String> getEtiquetasDelMesHastaHoy(LocalDate fechaInicio) {
	     ...
	     LocalDate fechaFin = fechaHoy.isBefore(fechaInicio.withDayOfMonth(fechaInicio.lengthOfMonth())) ? fechaHoy : fechaInicio.withDayOfMonth(fechaInicio.lengthOfMonth());
